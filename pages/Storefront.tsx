import { Play, Star, ArrowRight, Users, CheckCircle2, Layers, X, Plus, Award, Globe, Clapperboard, Sparkles, Loader2, Search, BookOpen, Video, Palette } from 'lucide-react';
...
const CourseCard = React.memo<{ course: Course; index: number; onBuy: (courseId: string) => void }>(({ course, index, onBuy }) => {
    ...
    return (
        <FadeIn delay={index * 50} className={`${isBundle ? 'lg:col-span-2' : ''}`}>
            <div className="group flex flex-col bg-white/5 rounded-3xl overflow-hidden border border-white/10 hover:border-white/20 hover:bg-white/[0.08] transition-all duration-300 hover:-translate-y-1 h-full backdrop-blur-sm">
                <Link to={`/course/${course.id}`} className={`relative overflow-hidden bg-neutral-900 block ${isBundle ? 'aspect-[2.2/1]' : 'aspect-[4/3]'}`}>
                    ...
                    <div className="absolute top-4 left-4 flex gap-2">
                        <span className="bg-white/10 backdrop-blur-md text-white text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wide border border-white/10">{course.type}</span>
                ...
                <div className="p-8 flex flex-col flex-grow">
                    <div className="flex items-center gap-3 mb-4 text-xs font-bold text-gray-500">
                        <div className="flex text-yellow-500 bg-yellow-500/10 px-2 py-1 rounded-md">
                            <Star size={12} fill="currentColor" className="mr-1"/>
                            <span>{course.rating || '4.8'}</span>
                        </div>
                        <span className="flex items-center gap-1 text-gray-400"><Users size={12}/> {course.totalStudents || 0} Students</span>
                        {isBundle && course.bundledCourses ? (
                            <span className="flex items-center gap-1 text-gray-400"><Layers size={12}/> {course.bundledCourses.length} Courses</span>
                        ) : (
                            <span className="flex items-center gap-1 text-gray-400"><Clapperboard size={12}/> {(course.chapters?.length || 0)} Lessons</span>
                        )}
                    </div>
                    <Link to={`/course/${course.id}`} className="block">
                        <h3 className="text-2xl font-bold text-white mb-3 group-hover:text-brand-400 transition-colors leading-tight">{course.title}</h3>
                        <p className="text-gray-400 mb-8 line-clamp-2 text-sm leading-relaxed">{course.description}</p>
                    </Link>
                    <div className="mt-auto flex items-center justify-between pt-6 border-t border-white/5">
                        <div className="flex flex-col">
                            <span className="text-xs text-gray-600 line-through font-medium">₹{Math.round(course.price * 1.5 / 100).toLocaleString()}</span>
                            <div className="text-2xl font-bold text-white">₹{(course.price / 100).toLocaleString()}</div>
                        </div>
                        <button
...
export const Storefront: React.FC = () => {
  ...
  return (
    <div className="bg-black font-sans text-white overflow-x-hidden">
      ...
      {/* ═══════════ HERO ═══════════ */}
      <section className="relative min-h-[90vh] flex flex-col items-center justify-center overflow-hidden bg-black pt-28 pb-16 px-4">
        {/* Ambient glow orbs */}
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          {/* Announcement pill */}
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/5 backdrop-blur-xl text-white text-xs font-bold uppercase tracking-widest mb-8 animate-fade-in-up hover:bg-white/10 transition duration-300 cursor-default">
          ...
          <h1 className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-black tracking-tighter mb-6 leading-[0.9] animate-fade-in-up" style={{ fontFamily: "var(--font-display)" }}>
            ...
            <br />
            <span className="text-white">of Filmmaking.</span>
          </h1>

          {/* Subtitle */}
          <p className="text-lg md:text-xl text-gray-400 mb-10 max-w-2xl mx-auto animate-fade-in-up leading-relaxed" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="group w-full sm:w-auto h-14 px-10 rounded-full bg-white/5 border border-white/20 hover:bg-white/10 text-white font-bold text-lg flex items-center justify-center gap-3 transition-all backdrop-blur-sm hover:scale-105"
            >
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 mt-10 animate-fade-in-up" style={{ animationDelay: '0.5s' }}>
            <span className="px-5 py-2.5 rounded-full bg-white/5 border border-white/10 text-sm font-bold text-gray-300 flex items-center gap-2 backdrop-blur-sm">
          ...
          {/* Academy seal */}
          <p className="text-xs text-gray-600 uppercase tracking-[0.3em] font-bold mt-8">Eyebuckz Academy</p>
        </div>
      </section>

      {/* ═══════════ SOCIAL PROOF TICKER ═══════════ */}
      <section className="bg-black border-b border-white/5 overflow-hidden py-6 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {VALUE_CARDS.map((card, i) => (
              <FadeIn key={i} delay={i * 100}>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-8 hover:bg-white/[0.08] hover:border-white/20 transition-all duration-300 h-full group">
      ...
      <section className="py-24 bg-[#111]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="text-center mb-16">
              <span className="inline-block px-4 py-1.5 bg-white/5 border border-white/10 text-gray-300 rounded-full font-bold tracking-wider uppercase text-xs mb-4">The Ecosystem</span>
              <h2 className="text-4xl md:text-5xl font-bold text-white" style={{ fontFamily: "var(--font-display)" }}>More Than Just Videos.</h2>
              <p className="text-gray-400 text-lg mt-4 max-w-2xl mx-auto">We provide the actual tools you need to execute the job.</p>
            </div>
          </FadeIn>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FadeIn>
              <div className="rounded-3xl overflow-hidden border border-white/10 bg-white/5 group hover:border-white/20 transition-all duration-300">
                <div className="h-56 bg-gradient-to-br from-brand-600 to-orange-500 relative overflow-hidden">
                  <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1626814026160-2237a95fc5a0?auto=format&fit=crop&q=80&w=800')] bg-cover bg-center opacity-20 group-hover:opacity-30 transition-opacity mix-blend-overlay" />
                  <div className="absolute bottom-6 left-6">
                    <Palette size={32} className="text-white/80" />
                  </div>
                </div>
                <div className="p-8">
                  <h3 className="text-2xl font-bold text-white mb-3">Premium Courses</h3>
                  <p className="text-gray-400 leading-relaxed mb-6">From cinematography fundamentals to advanced color grading — structured curricula with 100GB+ raw assets included.</p>
            ...
            <FadeIn delay={100}>
              <div className="rounded-3xl overflow-hidden border border-white/10 bg-white/5 group hover:border-white/20 transition-all duration-300">
                <div className="h-56 bg-gradient-to-br from-amber-500 to-yellow-400 relative overflow-hidden">
                  <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1526666923127-b2970f64b422?auto=format&fit=crop&q=80&w=800')] bg-cover bg-center opacity-20 group-hover:opacity-30 transition-opacity mix-blend-overlay" />
                  <div className="absolute bottom-6 left-6">
                    <Layers size={32} className="text-white/80" />
                  </div>
                </div>
                <div className="p-8">
                  <h3 className="text-2xl font-bold text-white mb-3">Digital Assets</h3>
                  <p className="text-gray-400 leading-relaxed mb-6">50+ LUTs, contract templates, business presets, and 6K RAW footage from RED and Arri Alexa cameras.</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="flex flex-col md:flex-row items-end justify-between mb-8 gap-6">
              <div>
                <h2 className="text-4xl font-bold text-white mb-2" style={{ fontFamily: "var(--font-display)" }}>Masterclass Catalog</h2>
                <p className="text-gray-400 text-lg">Choose your path. From cinematography to color grading.</p>
              </div>
              {/* Filters */}
            ...
            <div className="relative mb-10">
              ...
              <input
                ...
                className="w-full pl-12 pr-10 py-4 rounded-2xl border border-white/10 bg-white/5 text-white placeholder-gray-500 outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition text-lg backdrop-blur-sm"
              />
              {searchQuery && (
          ...
          {isLoading ? (
            ...
            <div className="text-center py-24 text-gray-500">No courses available yet.</div>
          ) : (
      ...
      {/* ═══════════ TESTIMONIALS ═══════════ */}
      <section className="py-24 bg-[#111] overflow-hidden border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="text-center mb-16">
              <h2 className="text-4xl font-bold mb-4 text-white" style={{ fontFamily: "var(--font-display)" }}>Don't just take our word for it.</h2>
              <p className="text-gray-400 text-lg">Join a community of serious filmmakers.</p>
            </div>
          </FadeIn>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((t, i) => (
              <FadeIn key={i} delay={i * 100}>
                <div className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-sm hover:bg-white/[0.08] hover:border-white/20 transition duration-300 h-full">
                  <div className="flex gap-1 text-yellow-500 mb-6">
                    {[...Array(5)].map((_, idx) => <Star key={idx} size={18} fill="currentColor"/>)}
                  </div>
                  <p className="text-gray-300 mb-8 leading-relaxed text-lg">"{t.text}"</p>
                  <div className="flex items-center gap-4 border-t border-white/5 pt-6">
                    <img src={t.image} className="w-12 h-12 rounded-full border-2 border-brand-600 object-cover" alt={t.name} />
                    <div>
                      <h4 className="font-bold text-white">{t.name}</h4>
                      <p className="text-xs text-gray-500 uppercase tracking-wide font-bold">{t.role}</p>
                    </div>
                  </div>
                </div>
              </FadeIn>
          ...
          <div className="space-y-4">
            {faqs.map((faq, idx) => (
              <FadeIn key={idx} delay={idx * 50}>
                <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden transition-all duration-300 hover:border-white/20 group">
                  <button
                    onClick={() => setOpenFaq(openFaq === idx ? null : idx)}
                    className="w-full flex items-center justify-between p-6 hover:bg-white/5 transition text-left"
                  >
                    <span className="font-bold text-white text-lg group-hover:text-brand-400 transition-colors">{faq.q}</span>
                    {openFaq === idx ? <X size={24} className="text-gray-500 shrink-0"/> : <Plus size={24} className="text-gray-500 shrink-0"/>}
                  </button>
                  {openFaq === idx && (
                    <div className="p-6 pt-0 border-t border-white/5 text-gray-400 leading-relaxed text-lg animate-slide-in">
                      {faq.a}
                    </div>
                  )}
                </div>
      ...
      <section className="py-32 bg-[#111] relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-4 relative z-10">
          <FadeIn>
            <div className="bg-white/5 border border-white/10 backdrop-blur-xl rounded-3xl p-12 md:p-16 text-center">
              ...
              <p className="text-xl text-gray-400 mb-10 max-w-xl mx-auto">Join thousands of filmmakers who have leveled up their craft. The camera is rolling.</p>
              <a href="#courses" className="inline-flex items-center gap-2 px-12 py-5 bg-brand-600 text-white rounded-full font-bold text-lg hover:bg-brand-500 transition shadow-2xl shadow-brand-600/30 hover:-translate-y-1 hover:scale-105 active:scale-95">
